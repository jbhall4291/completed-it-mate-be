    ...
    while (true) {
        try {
            ...
            if (e?.response?.data?.detail === 'Invalid page.') {
                throw new Error('END_OF_PAGINATION');
            }
    ...
    while (!limit || out.length < limit) {
        try {
            const res = await safeGet<{ results: List[] }>(
                `${RAWG}/games`,
                { page, page_size: PAGE_SIZE, ...params },
                `${label} p${page}`
            );
            ...
            out.push(...batch);
            process.stdout.write(
                `\r📄 [${label}] Listed ${Math.min(out.length, limit || out.length)}`
            );
            ...
            if (e.message === 'END_OF_PAGINATION') break;
            throw e;
        }
    }
...
/* ---------------------------------------------------
   Get announced and upcoming releases, next 6 months
---------------------------------------------------*/

async function fetchUpcoming(): Promise<List[]> {
    const today = new Date();
    const future = new Date(today);
    future.setMonth(today.getMonth() + 6);

    return fetchListSegment(
        150,
        {
            ordering: 'released',
            dates: `${d(today)},${d(future)}`,
        },
        'Upcoming_6mo'
    );
}
...
async function run() {
    ...
    const combined = [...latest, ...upcoming];
    console.log(`📦 Fetched ${combined.length} games (pre-dedupe)`);
    ...
    console.log(`📦 Will process ${deduped.length} games (post-dedupe)`);
    ...
    for (let i = 0; i < deduped.length; i += BATCH) {
        ...
        docs.push(...chunkDocs);
        process.stdout.write(
            `\r🔎 Detailed ${Math.min(i + chunk.length, deduped.length)}/${deduped.length}`
        );
        await delay(80);
    }
    process.stdout.write('\n');

    // Filter unwanted platforms, missing artwork etc
    ...
    const cleaned = docs.filter(({ doc, detail }) => {
        ...
        if (res.allowed === false) {
            ...
            if (!rejected[reason]) {
                rejected[reason] = { count: 0, samples: [] };
            }

            rejected[reason].count++;

            // keep a small sample per reason (e.g. first 5)
            if (rejected[reason].samples.length < 5) {
                rejected[reason].samples.push(doc.title || '(untitled)');
            }
